...
export class LighthouseCriteriaValidator {
  ...
  private validateLCP(): boolean {
    ...
    for (const file of htmlFiles) {
      ...
      const priorityImages = doc.querySelectorAll('img[fetchpriority="high"]');      const images = doc.querySelectorAll('img');
      if (images.length > 0 && priorityImages.length === 0) {
        console.warn(`❌ LCP: Missing fetchpriority="high" for images in ${file}`);
        hasOptimizations = false;
      }
      ...
      const blockingCSS = doc.querySelectorAll('link[rel="stylesheet"]:not([media])');
      if (blockingCSS.length > 2) { // Tolérance pour 1-2 fichiers CSS
  ...
  private validateCLS(): boolean {
    ...
    for (const file of htmlFiles) {
      ...
      for (const style of styles) {
        ...
        if (styleContent.includes('font-display:swap') || styleContent.includes('font-display: swap')) {
      ...
      for (const style of styles) {
        const styleContent = style.innerHTML;
        if (styleContent.includes('size-adjust:') || styleContent.includes('size-adjust: ')) {
          hasSizeAdjust = true;
          break;
        }
      }
  ...
  private validateINP(): boolean {
    ...
    for (const file of htmlFiles) {
      ...
      for (const script of scripts) {
        ...
        if (scriptContent.includes('scroll') && !scriptContent.includes('requestAnimationFrame')) {
          console.warn(`❌ INP: Script without requestAnimationFrame optimization in ${file}`);
          hasOptimizations = false;
        }

        // Vérifier utilisation de passive listeners
        if (scriptContent.includes('addEventListener') && !scriptContent.includes('passive: true')) {
      ...
      if (allElements.length > 1500) {
        console.warn(`❌ INP: DOM too complex (${allElements.length} elements) in ${file}`);
        hasOptimizations = false;
      }
    }
  ...
  private validateAssets(): boolean {
    ...
    if (totalSizeKB > 500) {
      console.warn(`❌ Assets: Total size too large (${totalSizeKB.toFixed(2)}KB)`);
      return false;
    }
  ...
  private validateImages(): boolean {
    ...
    for (const file of htmlFiles) {
      ...
      for (const img of images) {
        ...
        if (src && !src.includes('.webp') && !src.includes('.avif') && !src.includes('.svg')) {
  ...
  private validateCSS(): boolean {
    ...
    for (const file of cssFiles) {
      ...
      if (sizeKB > 100) {
        console.warn(`❌ CSS: File too large (${sizeKB.toFixed(2)}KB): ${file}`);
        hasOptimizations = false;
      }
    }
  ...
  private validateBestPractices(): boolean {
    ...
    for (const file of htmlFiles) {
      ...
      const canonical = doc.querySelector('link[rel="canonical"]');
      if (canonical && !canonical.getAttribute('href')?.startsWith('https://')) {
