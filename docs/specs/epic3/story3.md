...
- AC6: Les styles sont appliqués via la classe `prose` de Tailwind Typography, avec les personnalisations nécessaires dans `tailwind.config.cjs`.
...
- **Relevant Files:**
  - Files to Create/Modify: `src/layouts/ArticleLayout.astro` (ou un nom similaire pour le layout des articles), `tailwind.config.cjs` (pour la configuration de `@tailwindcss/typography` et des polices). Potentiellement un fichier CSS global si l'import des polices y est fait.
...
- **Coding Standards Notes:**
  - Déclarer les polices dans la configuration Tailwind de manière centralisée.
  - Personnaliser `prose` de manière modulaire dans `tailwind.config.cjs`.
  - _(Hint: See `docs/coding-standards.md` for full standards)_

## Tasks / Subtasks

- [ ] Installer et configurer le plugin `@tailwindcss/typography`.
- [ ] Importer les polices "Inter" et "JetBrains Mono" dans le projet (par exemple, via un fichier CSS global ou directement dans le layout principal).
- [ ] Configurer `tailwind.config.cjs` pour utiliser "Inter" comme police par défaut et "JetBrains Mono" pour les éléments `<code>` ou `pre`.
- [ ] Personnaliser les styles de `prose` dans `tailwind.config.cjs` pour ajuster les tailles de police, les interlignes, les couleurs des titres, des liens, du corps de texte, etc., afin qu'ils correspondent aux spécifications UI/UX pour les modes clair et sombre.
...
- [ ] Tester l'affichage d'un article MDX exemple pour vérifier l'application correcte de tous les styles typographiques.
