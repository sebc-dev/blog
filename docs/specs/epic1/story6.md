...
- [ ] Initialiser un nouveau projet Astro avec PNPM : `pnpm create astro@latest . --template empty --typescript strict --install --git no`. (Ajuster la commande si un template plus adapté est disponible ou si des options interactives sont préférées).
- [ ] Installer TailwindCSS avec l'intégration Astro : `pnpm astro add tailwind`. Suivre les instructions pour créer `tailwind.config.cjs` et `postcss.config.cjs`, et mettre à jour `astro.config.mjs`.
- [ ] Créer/Modifier `src/styles/global.css` (ou le nom choisi par l'intégration Tailwind) et y ajouter les directives Tailwind :
    ```css
    @tailwind base;
    @tailwind components;
    @tailwind utilities;
    ```
- [ ] Importer ce fichier CSS global dans un layout de base ou directement dans `src/pages/index.astro` pour test.
- [ ] Installer DaisyUI : `pnpm install -D daisyui@latest`.
- [ ] Configurer DaisyUI comme plugin dans `tailwind.config.cjs` :
    ```javascript
    // tailwind.config.cjs
    module.exports = {
      ...
      // (Optionnel pour le moment) daisyui: { themes: ["light", "dark"] }
    };
    ```
- [ ] Installer le plugin `@tailwindcss/typography`: `pnpm install -D @tailwindcss/typography`.
- [ ] Ajouter le plugin à `tailwind.config.cjs`:
    ```javascript
    // tailwind.config.cjs
    module.exports = {
    ...
    ```
- [ ] Créer une page d'accueil simple `frontend/src/pages/index.astro` qui utilise une classe Tailwind et un composant DaisyUI pour vérifier l'installation. Exemple :
    ```astro
    ---
    ...
    ```
- [ ] Lancer le serveur de développement Astro (`pnpm dev` depuis le répertoire `frontend/`) et vérifier que la page s'affiche correctement avec les styles appliqués.
- [ ] S'assurer que le fichier `pnpm-lock.yaml` est généré.
- [ ] Ajouter les fichiers générés (`astro.config.mjs`, `tailwind.config.cjs`, `postcss.config.cjs`, `tsconfig.json`, `src/styles/global.css`, `src/pages/index.astro`, `src/env.d.ts`, `package.json`, `pnpm-lock.yaml`) à Git.
